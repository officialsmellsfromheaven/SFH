...
Interactive usage:
    python scriptsdd_fragrantica_product_v5.py

Paste an exact Fragrantica perfume URL one by one.
Type EXIT, Q, or QUIT to stop.

Single URL:
    python scriptsdd_fragrantica_product_v5.py "https://www.fragrantica.com/perfume/Tom-Ford/Oud-Wood-1826.html"

Multiple URLs:
    python scriptsdd_fragrantica_product_v5.py "URL1" "URL2"

WHAT THIS VERSION IMPORTS FROM THE SUPPLIED FRAGRANTICA PAGE
- Product name + brand
- Gender
- Launch year
- Perfumer / nose (when published)
- Fragrance Family
- Main Accords
- Top / Middle / Base Notes
- Fragrantica description
- Fragrantica rating + rating votes
- Fragrantica review count
- Performance data when exposed in the page
- When-to-wear / season data when exposed in the page
- Primary product image

WEBSITE COMPATIBILITY
The importer writes both:
1. Direct fields used by the SFH product UI:
   topNotes, heartNotes, baseNotes, notes, fragranceFamily,
   mainAccordsList, rating, reviewCount, description, gender
2. A `fragrantica` object containing the richer source metadata.
...
- Existing SFH price, sizes, IDs, slugs, inventory, collections and local
...
- Source values are only written when reliably extracted.
...
def update_product(
    ...
    # Backup existing product.json before any mutation.
    ...
    # Keep the SFH category stable when it already exists. For a brand-new
    # imported product, derive only the basic men/women/unisex category from
    # the exact Fragrantica gender field.
    ...
    # --------------------------------------------------------
    # Fragrance metadata — overwrite stale source values when
    # the exact Fragrantica page supplied reliable values.
    # --------------------------------------------------------
    scalar_fields = [
        "gender",
        "fragranceFamily",
        ...
        "perfumer",
        "fragranticaRating",
        "fragranticaRatingVotes",
        "fragranticaReviewCount",
    ]
    ...
    # Keep Fragrantica's community rating separate from SFH's own customer
    # rating. This avoids presenting a third-party score as an SFH review.
    if metadata.get("rating") is not None:
        merged["fragranticaRating"] = metadata["rating"]
    if metadata.get("ratingVotes") is not None:
        merged["fragranticaRatingVotes"] = metadata["ratingVotes"]
    if metadata.get("reviewCount") is not None:
        merged["fragranticaReviewCount"] = metadata["reviewCount"]

    # Website-compatible nested note pyramid.
    ...
    # Keep an optional Fragrantica object for richer future UI use.
    fragrantica_data = dict(
        existing.get("fragrantica")
        if isinstance(existing.get("fragrantica"), dict)
        else {}
    )

    fragrantica_data["url"] = CURRENT_URL

    for key, value in metadata.get("fragrantica", {}).items():
        if value not in ("", None, []):
            fragrantica_data[key] = value

    merged["fragrantica"] = fragrantica_data
    merged["fragranticaUrl"] = CURRENT_URL

    # If available, also keep a direct source marker.
    merged["dataSource"] = "Fragrantica"
...
def process(url: str) -> bool:
    ...
    perfumers = extract_perfumer(soup)
    rating, rating_votes, review_count = extract_rating_and_counts(soup)
    ...
    print(f"Launch Year: {launch_year or 'Not published'}")
    print(f"Perfumer: {', '.join(perfumers) if perfumers else 'Not published'}")
    print(f"Fragrance Family: {family}")
    print("Main Accords: " + ", ".join(accords))
    print("Top Notes: " + ", ".join(top_notes))
    print("Middle Notes: " + ", ".join(heart_notes))
    print("Base Notes: " + ", ".join(base_notes))
    print(f"Rating: {rating if rating is not None else 'Not published'}")
    print(
        f"Rating Votes: "
        f"{rating_votes if rating_votes is not None else 'Not published'}"
    )
    print(
        f"Reviews: "
        f"{review_count if review_count is not None else 'Not published'}"
    )
    print(
        "Performance: "
        + (
    ...
    # Add direct performance/when-to-wear fields only when source data
    # actually exists. This keeps existing SFH values intact otherwise.
    if isinstance(performance, dict):
        if performance.get("longevity"):
            metadata["longevity"] = performance["longevity"]
        if performance.get("sillage"):
            metadata["projection"] = performance["sillage"]

    if isinstance(when_to_wear, dict):
        if isinstance(when_to_wear.get("dayNight"), dict):
            metadata["dayNight"] = when_to_wear["dayNight"]

        if isinstance(when_to_wear.get("seasons"), dict):
            metadata["seasons"] = when_to_wear["seasons"]

    # Optional direct fields are applied by update_product below.
    for optional_key in [
        "longevity",
        "projection",
        "dayNight",
    ...
    # Preserve source performance values in the merged product when found.
    update_product(
        folder,
        metadata,
        downloaded_image_url,
    ...
    # update_product handles the common fields above; add optional
    # performance fields in a second safe merge without losing anything.
    product_file = folder / "product.json"
    current_product = load_json(product_file)

    optional_updates = {}

    if metadata.get("longevity"):
        optional_updates["longevity"] = metadata["longevity"]

    if metadata.get("projection"):
        optional_updates["projection"] = metadata["projection"]

    if metadata.get("dayNight"):
        optional_updates["dayNight"] = metadata["dayNight"]

    if metadata.get("seasons"):
        optional_updates["seasons"] = metadata["seasons"]

    if optional_updates:
        current_product.update(optional_updates)
        product_file.write_text(
            json.dumps(
                current_product,
                ensure_ascii=False,
    ...
    print(f"Fragrantica URL: {CURRENT_URL}")

    if downloaded_image_url:
        print(f"Image: {downloaded_image_url}")
    elif existing_image_files:
        print("Image: Existing website images preserved")
